...
{
  //Setup Servos
  pitchServo.attach(pitchData.Pin); //the 3rd pin is the x axis servo
  yawServo.attach(yawData.Pin); //the 4th pin is the y axis servo
...
void states()
{
  ...
  //state 3
  if ((totalTime - timeAtLaunch) >= burnoutTimeLimit && accelData.z <= burnoutLimit && state == 2)
  {
    state = RECOVERY;
  }
}
...
void PID()
{
  ...
  pitchData.error = setpoint - pitchData.;
  ...
  yawData.error = setpoint - yawData.;
