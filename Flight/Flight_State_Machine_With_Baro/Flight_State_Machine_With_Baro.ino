...
#include <Servo.h>
//Acceleration Check Variables
int accelStatus;
bool accelBuffer;
Bmi088Accel accel(Wire,0x18);
float accelZ;
float launchLimit = 13;

//Altimeter Check Variables
int altStatus;
float prevAltitude;
...
void setup() {
  ...
  altStatus = bmp280.begin();
    if (altStatus < 0) {
    Serial.println("Altimeter Initialization Error");
    Serial.println(altStatus);
    while (1) {}
    }
...
bool accelCheck(){
  ...
  //Altitude Check
bool altCheck(){
    if (bmp280.getAltitude(Altitude)){
    ...
    if (prevAltitude>currentAltitude){
      return true;
    }
}
//TVC Function
...
//Acceleration check
accelCheck();
  //Launchpad to Active Control
if (accelCheck == true) { 
   TVC();
  }
altCheck();
//Active Control to Recovery
if (altCheck == true) {
   Parachutes();
  }
}
