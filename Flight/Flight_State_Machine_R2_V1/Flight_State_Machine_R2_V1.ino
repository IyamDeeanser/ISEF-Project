...
//initialize datalogging **still needs filewriting code**
void dataInitialize()
{
  ...
  int filenum = 0;
  while(true) {
    filenumber = to_string(filenum);
    if(SD.exists("Flight "+filenumber+".csv")) {
      filenumber = to_string(filenum);
      break;
    }
    else {
  ...
  // open the file
  myFile = SD.open(""Flight "+filenumber+".csv"", FILE_WRITE);
  
  // if the file opened okay, write to it:
  if (myFile) {
    Serial.print("Writing to Flight "+filenumber+".csv...");
    myFile.print("gyroX, ");
    myFile.print("gyroY, ");
    myFile.print("gyroZ, ");
    myFile.print("accelX, ");
    myFile.print("accelY, ");
    myFile.print("accelZ, ");
    myFile.print("pitch output, ");
    myFile.print("yaw output, ");
    myFile.print("gyroZ, ");
    myFile.print("state, ");
    myFile.println("time");
  ...
  // close the file:
    ...
    Serial.println("error opening Flight "+filenumber+".csv");
  }
}
...
//state changer **use case statement, add more states**
void states()
{
  //state 1
  if (accelCheck == false)
...
//launch detection **needs work** - how is there gonna be a 50ms delay when the loop is running at 10ms???? *answer: use the same datalogging delay as for this one
void launchDetect()
{  
  //Acceleration check
...
void dataLogging()
{
  ...
  //low datalogging rate
  if (state = 1){
    if(totalTime-lastLogTime > 1) {
      datalog = true;
  ...
  if (myFile && datalog) {
    ...
    myFile.print(pitchOutput-90);   myFile.print(", ");
    myFile.print(yawOutput-90);     myFile.print(", ");
